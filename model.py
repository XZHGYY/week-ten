    ...
    def build(self, embedding_file=None):
        ...
        with tf.variable_scope('rnn'):
            ...
            def create_cell():
                ...
                return cell
            multi_cell = tf.contrib.rnn.MultiRNNCell([create_cell() for i in range(self.rnn_layers)])
            self.state_tensor= multi_cell.zero_state(self.batch_size, tf.float32)
            outputs_tensor, self.outputs_state_tensor = tf.nn.dynamic_rnn(multi_cell, data, initial_state=self.state_tensor)
        ...
        with tf.variable_scope('softmax'):
            ...
            # Your Code here
            ##################
            W = tf.get_variable("W", [state_size, self.num_words], initializer=tf.random_normal_initializer(stddev=0.01))
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.reshape(self.Y, logits=logit)
